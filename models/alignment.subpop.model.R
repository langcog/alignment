...
speaker_type <- as.numeric(d2$vspeak) + 1
replier_type <- as.numeric(d2$vreply) + 1
...
num_subpops <- length(unique(speaker_type))
...
alignment_data <- list(NumPeople = num_people,
                       ...
                       ReplierType = replier_type)


fit <- stan(file = 'alignment.poplevels.stan', data = alignment_data, 
            iter = 1000, chains =1 )
