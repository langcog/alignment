...
childes.data <- left_join(childes.results,childes.userinfo) %>%
  ...
  filter(corpus == "Manchester", type == "Unstemmed") %>%
  mutate(Age = floor(Age/6)) %>%
  group_by(Child,Age,Speaker,marker) %>%
...
d4 <- d3 %>%
  ...
  mutate(Age = 6*as.numeric(Age)) %>%
  group_by(Child, Speaker, Age) %>%
