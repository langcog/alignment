...
transformed parameters {
  ...
  for (Person in 1:NumPeople) {
      alpha_person[Person] <- rho_person[Person] * n_person[Person]
      beta_person[Person] <- (1-rho_person[Person]) * n_person;
    }
}

model {

  alpha_pop ~ exponential(1);
  beta_pop ~ exponential(1);
