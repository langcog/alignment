...
This document contains code for analyzing CHILDES data with the WWW2016 alignment framework (HAM).
...
save(d3,file=paste0('../results/',currcorpus,'_liwc_www2016.RData'))
...
for (currcorpus in unique(childes.data$corpus)) {
#for (currcorpus in c('Suppes')) {
  load(paste0('../results/',currcorpus,'_liwc_www2016.RData'))
  if (exists('d3all')) {
    d3all %<>% bind_rows(d3)    
  } else {
    d3all <- d3
...
ggplot(aes(x = Age, y = mean, colour = Replier,
  ...
  #facet_wrap(~Child) + 
  geom_pointrange(aes(ymax = ci_upper, ymin = ci_lower),size=.5) +
