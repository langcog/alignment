...
result.files <- list.files(path = "../../Childes_results", 
                           pattern = '*Results.liwc2007_preps_byword.csv', all.files = FALSE)
...
childes.data$group <- childes.data$marker=='preps'
```
...
d4 <- d3all %>%
  ...
  group_by(Child, Speaker, Replier, Age, corpus, group) %>%
...
ggplot(aes(x = as.numeric(Age), y = mean, colour = paste(Replier,group),
           group = interaction(Child, Replier, group)), data = d4) +
           #), data=d4) + 
  facet_wrap(~Child) + 
  geom_point() +
...
ggplot(aes(x = Age, y = mean, colour = paste(Replier,group),
           group = interaction(Replier,marker) #), data = d4) +
            ), data=d5) + 
  #facet_wrap(~Child) + 
  geom_point() +
...
d5minus <- d5 %>%
  ...
  select(-Adult,-Child) %>%
  rename(Child=x,preps=group) %>%
  mutate(group=ifelse(preps,'Prepositions as a Category','Individual Prepositions')) %>%
  filter(!is.na(diff))
  
ggplot(aes(x = Age, y = diff, color=marker, group=marker), data=d5minus) + 
  geom_line(stat="smooth",method='loess',se=F,alpha=.3,size=.6) +
  geom_smooth(aes(group=preps),method = "loess") +
  #geom_hline(aes(yintercept=aamean),linetype='dashed',color="#3daf3a",size=1.2) +
  #scale_color_brewer(palette = "Set1") +
  theme_bw() + 
  geom_hline(yintercept=0,linetype='dotted',size=1) +
