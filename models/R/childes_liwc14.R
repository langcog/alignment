...
ggplot(aes(x = as.numeric(Age), y = mean, colour = paste(Speaker,Replier),
           group = interaction(Child, paste(Speaker,Replier))), data = d4) +
           #), data=d4) + 
  facet_wrap(~Child) + 
...
ggplot(aes(x = as.numeric(Age), y = mean, colour = paste(Speaker,Replier,sep='<-'),
...
ggplot(aes(x = Age, y = mean, colour = Speaker,
           #group = interaction(Child, Speaker)), data = d4) +
            ), data=d5) + 
  #facet_wrap(~Child) + 
...
bootd3df <- d3all %>%
  separate(sid, c("Speaker", "Replier", "Age"), sep = "_") %>%
  mutate(Speaker = adultchild(Speaker),Replier=adultchild(Replier)) %>%
  filter(Speaker!='ADU'|Replier!='ADU') %>%
  group_by(Speaker,Age,marker) %>%
  select(Speaker,Age,marker,model_eta) %>%
  summarize(model_eta=mean(model_eta)) %>%
  spread(Speaker,model_eta) %>%
  na.omit() %>%
  transmute(marker,diff=CHI-ADU) %>%
  group_by(marker) %>%
...
ggplot(aes(x=marker,y=mean),data=bootd3df) +
  ...
  labs(title='Increased alignment to children vs. to adults',
       #x='Child\'s Age (months)',
       y='Alignment difference (delta log-odds)')
