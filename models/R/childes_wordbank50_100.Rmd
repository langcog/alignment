...
d4 <- d3all %>%
  ...
  mutate(newgroup=ifelse(group=="1_50"|group=="51_100","content","liwc")) %>%
  group_by(Child, Speaker, Replier, Age, corpus, newgroup) %>%
...
ggplot(aes(x = as.numeric(Age), y = mean, colour = paste(Replier,newgroup),
           group = interaction(Child, Replier, newgroup)), data = d4) +
           #), data=d4) + 
  facet_wrap(~Child) + 
  geom_point() +
