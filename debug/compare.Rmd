...
d <- df[,list(ba=ba,nba=nba,bna=bna,nbna=nbna,
              ...
              pyalign=alignment,percentDiff=percentDiff,reciprocity=reciprocity,numUtterances=numUtterances, msgSentiment=msgSentiment, replySentiment=replySentiment),]
...
d2 <- d %>%
  ...
  summarize(convs=n(),mean=mean(lo1), reciprocity=reciprocity)

ggplot(d2,aes(x=vspeak,y=mean)) + geom_violin() + facet_wrap(~reciprocity)
```
